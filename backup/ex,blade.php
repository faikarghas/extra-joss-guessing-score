                    ...
                    <div class="flex flex-wrap">
                        <ul class="basis-1/2 border-r-[1px] border-[#383838]">
                            <li class="mb-2 flex items-center">
                                <img src="{{asset('/images/countries')}}/{{$match->flag_team1}}" class="h-[20px] xl:h-[30px]"/>
                                <p class="text-white font-sans ml-1.5 text-[15px] leading-tight">{{$match->team1}}</p>
                            </li>
                            <li class="flex items-center">
                                <img src="{{asset('/images/countries')}}/{{$match->flag_team2}}" class="h-[20px] xl:h-[30px]"/>
                        ...
                        <div class="basis-1/2 flex items-center justify-center">
                            ...
                            @auth
                            {{-- sudah login --}}
                                @if($match->match_status == "OPEN")
                                    @if ($d1 < $d2)
                                        {{-- belum expire --}}
                                        @foreach ($myguess as $item)
                                            @if($item->id_match == $match->id)
                                            {{-- sudah tebak skor --}}
                                            <ul class="score">
                                                <li class="flex items-center">
                                                    <p class="text-white font-sans mr-4 text-[22px] font-bold leading-tight">{{$item->guessing_score_a}}</p>
                                                </li>
                                                <li class="flex items-center">
                                                    <p class="text-white font-sans mr-4 text-[22px] font-bold leading-tight">{{$item->guessing_score_b}}</p>
                                                </li>
                                            </ul>
                                            <div class="bg-[#0085CF] text-white text-[12px] rounded-2xl py-1 px-2 w-[94px] text-center">Edit Skor</div>
                                            {{-- @else
                                            belum tebak skor
                                            <div data-idMatch={{$match->id}} data-flag1={{$match->flag_team1}} data-flag2={{$match->flag_team2}} data-team1={{$match->team1}} data-team2={{$match->team2}} class="btn-tebak bg-[#FF0000] text-white text-[12px] rounded-2xl py-1 px-2 w-[100px] text-center cursor-pointer">Tebak Skor</div> --}}
                                            @endif
                                        @endforeach
                                    @else
                                        {{-- sudah expire --}}
                                        @foreach ($myguess as $item)
                                            ...
                                            @else
                                            {{-- belum tebak skor dan waktu sudah expire--}}
                                            {{-- <ul class="score">
                                                <li class="flex items-center">
